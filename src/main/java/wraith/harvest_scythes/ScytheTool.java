        ...
        for (int x = -harvestRadius; x <= harvestRadius; ++x){
            for (int y = -1; y <= 1; ++y){
                for (int z = -harvestRadius; z <= harvestRadius; ++z){
                    ...
                    if (this.circleHarvest &&
                        ((y == -1 && newBlockPos.getManhattanDistance(blockPos.up()) > this.harvestRadius) ||
                        (y == 0 && newBlockPos.getManhattanDistance(blockPos) > this.harvestRadius) ||
                        (y == 1 && newBlockPos.getManhattanDistance(blockPos.down()) > this.harvestRadius))) {
                            continue;
                    }
